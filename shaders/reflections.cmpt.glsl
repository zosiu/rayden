...
vec3 trace_ray_r0(vec3 camera, vec3 ray, float t_min, float t_max, vec3 background_color,
...
vec3 trace_ray_r1(vec3 camera, vec3 ray, float t_min, float t_max, vec3 background_color,
                  Sphere[num_of_spheres] spheres, Light[3] lights) {
  Closest closest = closest_sphere(camera, ray, t_min, t_max, spheres);
...
vec3 trace_ray_r2(vec3 camera, vec3 ray, float t_min, float t_max, vec3 background_color,
                  Sphere[num_of_spheres] spheres, Light[3] lights) {
  Closest closest = closest_sphere(camera, ray, t_min, t_max, spheres);
...
void main() {
  ...
  vec3 pixel_color_on_canvas =
      trace_ray_r2(camera, ray, viewport.distance_from_camera, INF, background_color, spheres, lights);
