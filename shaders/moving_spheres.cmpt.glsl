...
const vec3 STARTING_CAMERA_ROTATION = vec3(-0.6, 0.1, 0.0);
const vec3 STARTING_CAMERA_POSITION = vec3(3.5, 7.0, -6.0);
...
void move_spheres() {
  SPHERES[0].center.x = SPHERES[0].center.x + sin(time) * 1.0 + 1.0;
  SPHERES[0].center.y = SPHERES[0].center.y + sin(time) * 4.0;

  SPHERES[1].center.y = SPHERES[1].center.y - cos(time * 2) * 3.0;
  SPHERES[1].center.z = SPHERES[1].center.z + sin(time * 2) * 3.0;

  SPHERES[2].center.x = SPHERES[2].center.x + cos(time * 2) * 6.0;
  SPHERES[2].center.z = SPHERES[2].center.z - sin(time) * 6.0;
}
