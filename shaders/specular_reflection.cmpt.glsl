...
vec3 trace_ray(vec3 camera, vec3 ray, float t_min, float t_max, vec3 background_color, Sphere[5] spheres,
               Light[3] lights) {
  float closest_t = INF;
  ...
  return closest_sphere.color *
         light_intensity(contact_point, sphere_normal_at_contact_point, -ray, closest_sphere.specular_exponent, lights);
}
...
void main() {
  ...
  Sphere spheres[5] = Sphere[5](Sphere(vec3(0.0, -1.0, 3.0), 1.0, vec3(1.0, 0.0, 1.0), 500),  //
                                ...
                                Sphere(vec3(0.0, -5001.0, 0.0), 5000.0, vec3(0.7, 0.7, 0.7), 1000));

  Light lights[3] = Light[3](Light(light_type_ambient, 0.2, vec3(0.0, 0.0, 0.0)), //
                             Light(light_type_point, 0.6, vec3(1.0, 2.0, 0.0)),   //
